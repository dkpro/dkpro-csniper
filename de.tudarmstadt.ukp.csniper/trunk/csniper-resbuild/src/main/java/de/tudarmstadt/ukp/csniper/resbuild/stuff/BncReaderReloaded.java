 ...
 * This is exactly the same as BncReader, but drops empty sentences.
 * 
 * @author Richard Eckart de Castilho
 */
public class BncReaderReloaded
		...
		extends TextExtractor
	{
			...
			throws SAXException
		{
			if (TAG_TITLE.equals(aName)) {
				...
				Sentence sentence = new Sentence(getJCas(), sentenceStart, getBuffer().length());
				if (JCasUtil.contains(getJCas(), sentence, Token.class)) {
					sentence.addToIndexes();
				}
				else {
					System.out.println("Dropping sentence (" + documentId + ")[" + sentenceStart + ","
							+ getBuffer().length() + "] because it doesn't contain any tokens.");
				}
				sentenceStart = -1;
			}
			else if (TAG_WORD.equals(aName) || TAG_CHARACTER.equals(aName)) {
